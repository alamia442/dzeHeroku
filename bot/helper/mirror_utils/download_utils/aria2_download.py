...
class AriaDownloadHelper(DownloadHelper):
    ...
    def add_download(self, link: str, path,listener):
        ...
        elif link.lower().endswith(".torrent") and not link.lower().startswith("http"):
            download = aria2.add_torrent(link, None, {'dir': path})
        elif search('debrid', link):
            download = aria2.add_uris([link], {'dir': path,'all-proxy': 'http://127.0.0.1:8128'})
        else:
