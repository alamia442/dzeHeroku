...
from bot import bot, botStartTime, DOWNLOAD_DIR
...
def update_all_messages():
    ...
    msg += f"<b>CPU:</b> {cpu_percent()} %" \
           f"  |  <b>DISK:</b> {get_readable_file_size(disk_usage(DOWNLOAD_DIR).free)} " \
           f"\n<b>RAM:</b> {virtual_memory().percent} %" \
           f"  |  <b>UPTIME:</b> {get_readable_time((time.time() - botStartTime))}"
    with status_reply_dict_lock:
...
def sendStatusMessage(msg, bot):
    ...
    progress += f"<b>CPU:</b> {cpu_percent()} %" \
           f"  |  <b>DISK:</b> {get_readable_file_size(disk_usage(DOWNLOAD_DIR).free)} " \
           f"\n<b>RAM:</b> {virtual_memory().percent} %" \
           f"  |  <b>UPTIME:</b> {get_readable_time((time.time() - botStartTime))}"
    with status_reply_dict_lock:
